...
//		uServ.insertUser(user4, account4, assest4);
		...
		return ResponseEntity.status(201).body("Successfully Inserted Users");
	}
...
//	}
	...
	public ResponseEntity<User> updateProfile(@RequestBody User user) {
		...
		if (email.isPresent() | email.isEmpty() | username.isPresent() | username.isEmpty()) {
			return ResponseEntity.badRequest().build();
		}
	...
	@GetMapping("/profileinfo/{name}")
	public ResponseEntity<User> getProfileInfo(@PathVariable("name") String user) {
		return ResponseEntity.status(201).body(uServ.getUserByHuntername(user));
	}
